...
// GET /enums - Status & Priority Optionen
...
// PATCH /:id/priority - Priorität ändern
...
// POST /:id/notes - Notiz hinzufügen
...
// POST /:id/meetings - Meeting hinzufügen
...
// POST /bulk-import - Bulk Import von Google Places
router.post('/bulk-import', (req, res) => {
  try {
    ...
    res.json({ success: true, imported: result.imported.length, skipped: result.skipped.length, details: result });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
});
...
function getStatusLabel(status) {
  const labels = { new: 'Neu', contacted: 'Kontaktiert', meeting_scheduled: 'Meeting geplant', meeting_done: 'Meeting durchgeführt', negotiating: 'In Verhandlung', won: 'Gewonnen', lost: 'Verloren' };
  return labels[status] || status;
}

function getPriorityLabel(priority) {
  const labels = { low: 'Niedrig', medium: 'Mittel', high: 'Hoch', hot: '🔥 Hot' };
